...
public class LNRNotificationManager: NSObject {
     ...
     */
    public func showNotification(title: String, body: String?, icon: UIImage?, onTap: LNRNotificationOperationCompletionBlock?) {
        dispatch_async(dispatch_get_main_queue()) {
            if self.isNotificationActive {
                self.dismissActiveNotification({ () -> Void in
                    self.showNotification(title, body: body, icon: icon, onTap: onTap)
                })
            } else {
                let notification = LNRNotificationView(title: title, body: body, icon: icon, duration: self.notificationsDefaultDuration, onTap: onTap, position: self.notificationsPosition, notificationManager: self)
